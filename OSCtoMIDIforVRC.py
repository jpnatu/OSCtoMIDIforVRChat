...
class Application(tk.Frame):
    ...
    def widget_left(self):
        ...
        labelframe = tk.LabelFrame(frame2, labelanchor="nw", text="Settings")
        labelframe.pack(side=tk.BOTTOM, fill=tk.BOTH, expand=True, padx=10, pady=10)

        # ハンドサイン選択
        label3_1 = tk.Label(labelframe, text="割り当てるハンドサイン")
        label3_1.pack(side=tk.TOP, anchor=tk.W, padx=5)
        cb3_1 = ttk.Combobox(labelframe, state="readonly",value=handsign)
        ...
        # 送信チャンネル選択
        label3_3 = tk.Label(labelframe, text="送るチャンネル")
        label3_3.pack(side=tk.TOP, anchor=tk.W, padx=5)
        cb3_2 = ttk.Combobox(labelframe, state="readonly",value=channelvalue)
        ...
        # ノート選択
        label3_4 = tk.Label(labelframe, text="ノート番号")
        label3_4.pack(side=tk.TOP, anchor=tk.W, padx=5)
        entNote_R = tk.Entry(labelframe)
        entNote_R.pack(side=tk.TOP, fill=tk.X)
        
        # JSONかきこみ
        def Writetojson():
            keylist["Lefthand"][cb3_1.current()] = \
        ...
        btn2 = tk.Button(labelframe,text="Write and Reload",command=Writetojson)
